<x-app-layout>
    <x-slot:title>Tambah Kategori Pengeluaran</x-slot>
    ...
    <div class="border-b border-gray-100 pb-5 mb-5">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-lg font-semibold text-gray-900">Tambah Kategori Pengeluaran</h2>
                <p class="text-sm text-gray-500 mt-0.5">
                    Tambahkan data kategori pengeluaran baru
                </p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden max-w-2xl">
        <form method="POST" action="{{ route('kategori-pengeluaran.store') }}" class="p-6">
            @csrf

            <div class="mb-6">
                <x-input-label for="nama" value="Nama Kategori" class="mb-1.5" />
                <x-text-input id="nama" name="nama" type="text" class="block w-full"
                    :value="old('nama')" required autofocus autocomplete="off" placeholder="Contoh: Operasional, ATK, dll..." />
